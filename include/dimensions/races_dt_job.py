...
def elaborate_drivers_dt(
    spark: SparkSession, sessions_parquet_file_path: str
) -> DataFrame | None:

    try:
        races_df = spark.read.parquet(sessions_parquet_file_path)
        if not races_df:
...
def execute_races_dt_job() -> None:
    ...
    teams_df = elaborate_drivers_dt(
        spark=spark, sessions_parquet_file_path="./include/data/sessions.parquet"
    )
